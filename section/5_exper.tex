...
We extract the original data structures and caller-callees, 
...
For example, some bugs can only be triggered on Windows, and some bugs depend on .Net libraries. 
For example, we have bugs related to GUI widgets, and it is too complex to re-implement these GUI widgets. 
We also have bugs, in which buggy loops traverse graphs. 
Without repeating these bugs, it is too difficult to figure out graph structures just through code reading.  
Overall, these 18 bugs cover a wide variety of inefficiency root causes, as 
shown in Table~\ref{tab:benchmarks}. 

Second, we evaluate \Tool on 21 out of the 42 detected by Toddler \cite{Alabama}.
We use all benchmarks that are reported with bug-triggering inputs and fixed by developers. 
...
We re-implement these basic data structures following the recent version of openjdk. 
...
For two benchmarks, there are only 4 loops in our re-implemented versions. For all other benchmarks, there are no less than 5 loops. 
...
For all benchmarks from Toddler project, 
statistical debugging ranks the real root-cause loop as Top 1 or Top 2 (for cases where there are two root-cause loops). 
During our accuracy experiments, we will apply LDoctor to top 5 loops identified by statistical debugging. 
For some re-implemented bugs, there may be less than 5 loops returned by statistical debugging. 
The number of loops we analyze for the first benchmark set is 64, and the number of loops we analyze for Toddler benchmark set is 103. 
...
\Tool.
For MySQL\#27287, the root-cause loop
