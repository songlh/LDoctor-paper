...
At high level, we will compare the computation of different iterations
from one loop instance and that of different loop instances from 
one static loop, 
and judge whether there exists redundancy.
Specifically, our design needs to answer several questions.
...
We will use both static optimization and dynamic sampling
to reduce the amount of data that we need to record and compare, lowering
time and spatial overhead. Details will be presented in 
...
%applied to many \texttt{source} strings slightly different from each other
%with a lot of
%redundant computation.
%We use it to demonstrate how we identify \texttt{input}s below.
...
off-line debugging or during production runs, 
...
iterations; in different iterations, different sets of source instructions
...
Recording values returned by every source instruction would lead to 
huge run-time overhead. To lower the overhead,
...
computation as long as the sampling rate is not too sparse, which
we will evaluate
...
not change throughout one loop instance
or multiple loop instances. 
Since we implement \Tool at LLVM byte-code level, the majority of this
analysis is already done by LLVM, which lifts loop-invariant memory
accesses out of loops. 
...
its value once during each loop, not each iteration.
However, LLVM does not apply its loop-invariant analysis to heap and global
variables, so \Tool conducts some extra analysis --- it identifies those
memory reads \texttt{read(v)}, where the address of \texttt{v} 
is a loop invariant and no instructions 
inside the loop can possibly write to that address. 
For example, \Tool can identify that the values of
\texttt{aNode->localName} and \texttt{aNode->namespaceURI} do not change
throughout one loop instance
in Figure~\ref{fig:Mozilla477564}. This analysis is sound but not complete.
\Tool may miss some loop invariants. 
...
If a loop iteration's \texttt{input} set contains a loop-induction
...
Of course, this optimization could lead to false positives, reporting
non-redundant loop computation as redundant.
This may happen when the corresponding
memory locations are modified across different loop instances.
In that case, although different loop instances work on variables read from
similar memory locations, the values they read are different.
