...
{\color{red} We only check redundancy between consecutive loop instances.
This could bring false negatives, but checking redundancy between every pairs of loop instances is time consuming.}
...
{\color{red} We calculate the redundancy for one input instruction $I$ by using 1 to subtract the normalized
edit-distance between the two sequences of values returned by $I$ in the two
loop instances, denoted by $SeqA$ and $SeqB$. 
...
outside the loop.
{\color{red} We may have false positives here, 
since heap/global variables could be changed by a different thread. 
However, concurrent programs are out of scope for current design of \Tool. 
} 
\comment{
...
Section \ref{sec:study}, as there are often only one or two natural fix 
