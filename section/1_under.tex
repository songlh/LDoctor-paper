...
As an example, Mozilla JavaScript engine used to use a linked list 
\texttt{script}
to store debugging information
for every bytecode instruction in a JavaScript file. 
Later on, people complained that Mozilla browsers were very slow in handling
large JavaScript files.
It turned out that those large
JavaScript files filled the linked list \texttt{script} with
tens of thousands of nodes. Repeated search through the
long \texttt{script} list then led to poor performance.
To fix this problem, 
...
(S: conditionally skip the loop; C: change the data structure)
}
\label{tab:root}
\end{table*}
